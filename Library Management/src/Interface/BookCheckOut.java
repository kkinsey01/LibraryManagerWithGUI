...
public class BookCheckOut {
    ...
    public BookCheckOut() {
        ...
        frame = new JFrame();
        frame.setTitle("Book Check Out");
        frame.add(panel);
        frame.setSize(new Dimension(650, 350));
        frame.setLocationRelativeTo(null);
        frame.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);

        JLabel prompt1 = new JLabel("Select account search criteria, then click OK");
        ...
        JLabel prompt3 = new JLabel("Select book search criteria, then click OK");
        ...
        yCord += 60;
        checkOut.setBounds(265, yCord, 10 + checkOut.getPreferredSize().width, 8 + checkOut.getPreferredSize().height);
    ...
    public void doStuff() {
        ...
        if (accountData[0] == null) {
            ...
            if (bookData[bookData.length - 1].equals("0")) {
                JLabel returnData = new JLabel("Book is already checked out!");
                returnData.setFont(new Font("Serif", Font.BOLD, 24));
                returnData.setAlignmentX(Component.CENTER_ALIGNMENT);
                returnData.setForeground(Color.RED);
                panel.add(returnData);
                ...
                String sql = "UPDATE books SET Availability=0, Rented_to=" + accountData[0] + " WHERE UPPER(Title)='" + bookData[2] + "'";
                connection.update(sql);
                int numRentals = Integer.parseInt(accountData[3]) + 1;
                String accSql = "UPDATE customer SET Number_of_Rentals=" + numRentals + ", Rentals=" + bookData[0] + " WHERE UPPER(Name)='" + accountData[1] + "'";
                connection.update(accSql);
            }
            JLabel success = new JLabel("Book successfully checked out!");
